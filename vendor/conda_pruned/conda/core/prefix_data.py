...
from ..common.compat import JSONDecodeError, itervalues, odict, scandir, string_types, with_metaclass
...
class PrefixDataType(type):
    ...
    def __call__(cls, prefix_path, pip_interop_enabled=None):
        if prefix_path in PrefixData._cache_:
            ...
            prefix_data_instance = super(PrefixDataType, cls).__call__(prefix_path, pip_interop_enabled)
...
class PrefixData(object):
    ...
    def load(self):
        ...
        if lexists(_conda_meta_dir):
            conda_meta_json_paths = (p for p in (entry.path for entry in scandir(_conda_meta_dir)) if p[-5:] == ".json")
    ...
    def insert(self, prefix_record):
        ...
        prefix_record_json_path = join(self.prefix_path, "conda-meta", self._get_json_fn(prefix_record))
    ...
    def _python_pkg_record(self):
        ...
        return next((prefix_record for prefix_record in itervalues(self.__prefix_records) if prefix_record.name == "python"), None)
    ...
    def _load_site_packages(self):
        ...
        for conda_anchor_file in clobbered_conda_anchor_files:
            ...
            prefix_rec_json_path = join(self.prefix_path, "conda-meta", "%s.json" % extracted_package_dir)
            try:
...
def get_conda_anchor_files_and_records(site_packages_short_path, python_records):
    ...
    matcher = re.compile(r"^%s/[^/]+(?:%s)$" % (re.escape(site_packages_short_path), r"|".join(re.escape(fn) for fn in anchor_file_endings))).match
...
def delete_prefix_from_linked_data(path):
    ...
    linked_data_path = next((key for key in sorted(PrefixData._cache_, reverse=True) if path.startswith(key)), None)
    if linked_data_path:

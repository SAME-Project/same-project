...
class BasicClobberError(ClobberError):
    def __init__(self, source_path, target_path, context):
        ...
        if context.path_conflict == PathConflict.prevent:
            message += "Conda no longer clobbers existing paths without the use of the " "--clobber option\n."
        super(BasicClobberError, self).__init__(message, context.path_conflict, target_path=target_path, source_path=source_path)
...
class CondaHTTPError(CondaError):
    def __init__(self, message, url, status_code, reason, elapsed_time, response=None, caused_by=None):
        from .common.url import maybe_unquote
        ...
        if isinstance(reason, string_types):
            reason = reason.upper()
        super(CondaHTTPError, self).__init__(
            message, url=url, status_code=status_code, reason=reason, elapsed_time=elapsed_time, response_details=response_details, caused_by=caused_by
        )
...
class CorruptedEnvironmentError(CondaError):
    def __init__(self, environment_location, corrupted_file, **kwargs):
        ...
        super(CorruptedEnvironmentError, self).__init__(message, environment_location=environment_location, corrupted_file=corrupted_file, **kwargs)
...
class InvalidVersionSpec(InvalidSpec):
    def __init__(self, invalid_spec, details):
        ...
        super(InvalidVersionSpec, self).__init__(message, invalid_spec=invalid_spec, details=details)


class InvalidMatchSpec(InvalidSpec):
    def __init__(self, invalid_spec, details):
        message = "Invalid spec '%(invalid_spec)s': %(details)s"
        super(InvalidMatchSpec, self).__init__(message, invalid_spec=invalid_spec, details=details)
...
def maybe_raise(error, context):
    if isinstance(error, CondaMultiError):
        ...
        if (
            ...
            or (clobber_errors and context.path_conflict == PathConflict.prevent and not context.clobber)
            or other_errors
        ):
            raise error
        elif (safety_errors and context.safety_checks == SafetyChecks.warn) or (clobber_errors and context.path_conflict == PathConflict.warn and not context.clobber):
...
def print_conda_exception(exc_val, exc_tb=None):
    ...
    if context.debug or context.verbosity > 2 or (not isinstance(exc_val, DryRunExit) and context.verbosity > 0):
...
#             message_builder.append("")
#             message_builder.append("An unexpected error has occurred. Conda has prepared the above report.")
#             message_builder.append("")
...
#             message_builder.append("A reportable application error has occurred. Conda has prepared the above report.")
#             message_builder.append("")
#             self.write_out("\n".join(message_builder))
...
#             response = requests.post(url, headers=headers, timeout=_timeout, data=data, allow_redirects=False)

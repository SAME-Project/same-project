...
if boolify(os.environ.get("CONDA_TIMED_LOGGING")):
    _FORMATTER = DeltaSecondsFormatter("%(relative_created_secs) 7.2f %(delta_secs) 7.2f " "%(levelname)s %(name)s:%(funcName)s(%(lineno)d): %(message)s")
else:
...
class SwallowBrokenPipe(ContextDecorator):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_val and isinstance(exc_val, EnvironmentError) and getattr(exc_val, "errno", None) and exc_val.errno in (EPIPE, ESHUTDOWN):
            return True
...
def attach_stderr_handler(level=WARN, logger_name=None, propagate=False, formatter=None):
    ...
    logr = getLogger(logger_name)
    old_stderr_handler = next((handler for handler in logr.handlers if handler.name == "stderr"), None)
...
#             try:
#                 self.pbar = tqdm(desc=description, bar_format=bar_format, ascii=True, total=1, file=sys.stdout)
#             except EnvironmentError as e:
#                 if e.errno in (EPIPE, ESHUTDOWN):
#                     self.enabled = False
#                 else:

...
from .compat import binary_type, isiterable, iteritems, itervalues, odict, primitive_types, scandir, string_types, text_type, with_metaclass
...
class ConfigurationLoadError(ConfigurationError):
    def __init__(self, path, message_addition="", **kwargs):
        ...
        super(ConfigurationLoadError, self).__init__(message + message_addition, path=path, **kwargs)
...
class MultipleKeysError(ValidationError):
    def __init__(self, source, keys, preferred_key):
        self.source = source
        self.keys = keys
        msg = "Multiple aliased keys in file %s:\n" "%s" "Must declare only one. Prefer '%s'" % (source, pretty_list(keys), preferred_key)
...
class InvalidTypeError(ValidationError):
    def __init__(self, parameter_name, parameter_value, source, wrong_type, valid_types, msg=None):
        ...
        if msg is None:
            msg = "Parameter %s = %r declared in %s has type %s.\n" "Valid types:\n%s" % (parameter_name, parameter_value, source, wrong_type, pretty_list(valid_types))
...
class InvalidElementTypeError(InvalidTypeError):
    def __init__(self, parameter_name, parameter_value, source, wrong_type, valid_types, index_or_key):
        qualifier = "at index" if isinstance(index_or_key, int) else "for key"
        msg = (
            "Parameter %s declared in %s has invalid element %r %s %s.\n"
            "Valid element types:\n"
            "%s." % (parameter_name, source, parameter_value, qualifier, index_or_key, pretty_list(valid_types))
        )
        super(InvalidElementTypeError, self).__init__(parameter_name, parameter_value, source, wrong_type, valid_types, msg=msg)
...
class CustomValidationError(ValidationError):
    def __init__(self, parameter_name, parameter_value, source, custom_message):
        ...
        super(CustomValidationError, self).__init__(parameter_name, parameter_value, source, msg=msg)
...
class EnvRawParameter(RawParameter):
    ...
    def value(self, parameter_obj):
        ...
        if hasattr(parameter_obj, "string_delimiter"):
            ...
            return tuple(EnvRawParameter(EnvRawParameter.source, self.key, v) for v in (vv.strip() for vv in self._raw_value.split(string_delimiter)) if v)
        else:
    ...
    def make_raw_parameters(cls, appname):
        ...
        raw_env = dict((k.replace(keystart, "", 1).lower(), v) for k, v in iteritems(environ) if k.startswith(keystart))
...
class ArgParseRawParameter(RawParameter):
    ...
    def value(self, parameter_obj):
        ...
        if isiterable(self._raw_value):
            ...
            for i in range(len(self._raw_value)):
                children_values.append(ArgParseRawParameter(self.source, self.key, self._raw_value[i]))
    ...
    def make_raw_parameters(cls, args_from_argparse):
        return super(ArgParseRawParameter, cls).make_raw_parameters(ArgParseRawParameter.source, args_from_argparse)
...
class YamlRawParameter(RawParameter):
    ...
    def __init__(self, source, key, raw_value, key_comment):
        ...
        if isinstance(self._raw_value, CommentedSeq):
            ...
            children_values = []
            for i in range(len(self._raw_value)):
                children_values.append(YamlRawParameter(self.source, self.key, self._raw_value[i], value_comments[i]))
            ...
            self._value_flags = dict((k, ParameterFlag.from_string(v)) for k, v in iteritems(value_comments) if v is not None)
            children_values = {}
    ...
    def _get_yaml_list_comments(value):
        ...
        raw_comment_lines = tuple(
            excepts(
                (AttributeError, IndexError, KeyError, TypeError), lambda q: YamlRawParameter._get_yaml_list_comment_item(items[q]), lambda _: None  # default value on exception
            )(q)
    ...
    def _get_yaml_map_comments(value):
        return dict(
            (key, excepts((AttributeError, KeyError), lambda k: value.ca.items[k][2].value.strip() or None, lambda _: None)(key)) for key in value  # default value on exception
        )

    @classmethod
    def make_raw_parameters(cls, source, from_map):
        if from_map:
            return dict((key, cls(source, key, from_map[key], cls._get_yaml_key_comment(from_map, key))) for key in from_map)
        return EMPTY_MAP
    ...
    def make_raw_parameters_from_file(cls, filepath):
        with open(filepath, "r") as fh:
            try:
                ...
                raise ConfigurationLoadError(filepath, "  reason: invalid yaml at line %(line)s, column %(column)s", line=mark.line, column=mark.column)
            except ReaderError as err:
                raise ConfigurationLoadError(filepath, "  reason: invalid yaml at position %(position)s", position=err.position)
...
class DefaultValueRawParameter(RawParameter):
    ...
    def __init__(self, source, key, raw_value):
        ...
        if isinstance(self._raw_value, Mapping):
            ...
            children_values = []
            for i in range(len(self._raw_value)):
                children_values.append(DefaultValueRawParameter(self.source, self.key, self._raw_value[i]))
            self._value = tuple(children_values)
        elif isinstance(self._raw_value, ConfigurationObject):
            self._value = self._raw_value
            for attr_name, attr_value in vars(self._raw_value).items():
                self._value.__setattr__(attr_name, DefaultValueRawParameter(self.source, self.key, attr_value))
...
def load_file_configs(search_path):
    ...
    def _dir_loader(fullpath):
        for filepath in sorted(p for p in (entry.path for entry in scandir(fullpath)) if p[-4:] == ".yml" or p[-5:] == ".yaml"):
...
class LoadedParameter(object):
    ...
    def collect_errors(self, instance, typed_value, source="<<merged>>"):
        ...
        if not isinstance(typed_value, self._type):
            errors.append(InvalidTypeError(self._name, typed_value, source, type(self.value), self._type))
    ...
    def _typify_data_structure(value, source, type_hint=None):
        if isinstance(value, Mapping):
            ...
            return value
        elif isinstance(value, string_types) and isinstance(type_hint, type) and issubclass(type_hint, string_types):
    ...
    def _first_important_matches(matches):
        idx = first(enumerate(matches), lambda x: LoadedParameter._match_key_is_important(x[1]), apply=lambda x: x[0])
...
class MapLoadedParameter(LoadedParameter):
    ...
    def merge(self, matches):
        ...
        relevant_matches_and_values = tuple((match, match.value) for match in LoadedParameter._first_important_matches(matches))

        for match, value in relevant_matches_and_values:
            if not isinstance(value, Mapping):
                raise InvalidTypeError(self.name, value, match.source, value.__class__.__name__, self._type.__name__)

        # map keys with important values
        def key_is_important(match, key):
            return match.value_flags.get(key) == ParameterFlag.final

        important_maps = tuple(dict((k, v) for k, v in iteritems(match_value) if key_is_important(match, k)) for match, match_value in relevant_matches_and_values)

        # map each value by recursively calling merge on any entries with the same key
        merged_values = frozendict(merge_with(lambda value_matches: value_matches[0].merge(value_matches), (match_value for _, match_value in relevant_matches_and_values)))

        # dump all matches in a dict
        # then overwrite with important matches
        merged_values_important_overwritten = frozendict(merge(concatv([merged_values], reversed(important_maps))))

        # create new parameter for the merged values
        return MapLoadedParameter(self._name, merged_values_important_overwritten, self._element_type, self.key_flag, self.value_flags, validation=self._validation)
...
class SequenceLoadedParameter(LoadedParameter):
    ...
    def merge(self, matches):
        ...
        def get_marked_lines(match, marker):
            return tuple(line for line, flag in zip(match.value, match.value_flags) if flag is marker) if match else ()

        top_lines = concat(get_marked_lines(m, ParameterFlag.top) for m, _ in relevant_matches_and_values)
        ...
        bottom_lines = concat(get_marked_lines(m, ParameterFlag.bottom) for m, _ in reversed(relevant_matches_and_values))
        ...
        return SequenceLoadedParameter(self._name, merged_values, self._element_type, self.key_flag, self.value_flags, validation=self._validation)
...
class ObjectLoadedParameter(LoadedParameter):
    ...
    def __init__(self, name, value, element_type, key_flag, value_flags, validation=None):
        ...
        """
        self._element_type = element_type
        super(ObjectLoadedParameter, self).__init__(name, value, key_flag, value_flags, validation)

    def collect_errors(self, instance, typed_value, source="<<merged>>"):
        errors = super(ObjectLoadedParameter, self).collect_errors(instance, typed_value, self.value)
    ...
    def merge(self, matches):
        ...
        relevant_matches_and_values = tuple(
            (match, {k: v for k, v in vars(match.value).items() if isinstance(v, LoadedParameter)}) for match in LoadedParameter._first_important_matches(matches)
        )
        ...
        # create new parameter for the merged values
        return ObjectLoadedParameter(self._name, object_copy, self._element_type, self.key_flag, self.value_flags, validation=self._validation)
...
class Parameter(object):
    ...
    def get_all_matches(self, name, names, instance):
        ...
        for filepath, raw_parameters in iteritems(instance.raw_data):
            match, error = ParameterLoader.raw_parameters_from_single_source(name, names, raw_parameters)
...
class PrimitiveParameter(Parameter):
    ...
    def load(self, name, match):
        return PrimitiveLoadedParameter(name, self._type, match.value(self._element_type), match.keyflag(), match.valueflags(self._element_type), validation=self._validation)
...
class MapParameter(Parameter):
    ...
    def load(self, name, match):
        ...
        if value is None:
            return MapLoadedParameter(name, frozendict(), self._element_type, match.keyflag(), frozendict(), validation=self._validation)

        if not isinstance(value, Mapping):
            raise InvalidTypeError(name, value, match.source, value.__class__.__name__, self._type.__name__)
        ...
        return MapLoadedParameter(name, frozendict(loaded_map), self._element_type, match.keyflag(), match.valueflags(self._element_type), validation=self._validation)
...
class SequenceParameter(Parameter):
    ...
    def load(self, name, match):
        ...
        if value is None:
            return SequenceLoadedParameter(name, tuple(), self._element_type, match.keyflag(), tuple(), validation=self._validation)
        ...
        return SequenceLoadedParameter(name, tuple(loaded_sequence), self._element_type, match.keyflag(), match.valueflags(self._element_type), validation=self._validation)
...
class ObjectParameter(Parameter):
    ...
    def load(self, name, match):
        ...
        value = match.value(self._element_type)
        if value is None:
            return ObjectLoadedParameter(name, None, self._element_type, match.keyflag(), None, validation=self._validation)
        ...
        object_parameter_attrs = {
            attr_name: parameter_type for attr_name, parameter_type in vars(self._element_type).items() if isinstance(parameter_type, Parameter) and attr_name in value.keys()
        }
        ...
        return ObjectLoadedParameter(name, object_copy, self._element_type, match.keyflag(), match.valueflags(self._element_type), validation=self._validation)
...
class ParameterLoader(object):
    ...
    def _raw_parameters_from_single_source(self, raw_parameters):
        return ParameterLoader.raw_parameters_from_single_source(self.name, self.names, raw_parameters)
    ...
    def raw_parameters_from_single_source(name, names, raw_parameters):
        ...
        if numkeys == 0:
            ...
            return matches[name], MultipleKeysError(raw_parameters[next(iter(keys))].source, keys, name)
        else:
            return None, MultipleKeysError(raw_parameters[next(iter(keys))].source, keys, name)
...
class ConfigurationType(type):
    ...
    def __init__(cls, name, bases, attr):
        ...
        cls.parameter_names = tuple(p._set_name(name) for name, p in iteritems(cls.__dict__) if isinstance(p, ParameterLoader))
...
class Configuration(object):
    ...
    def _set_argparse_args(self, argparse_args):
        ...
        if hasattr(argparse_args, "__dict__"):
            ...
            self._argparse_args = AttrDict((k, v) for k, v, in iteritems(vars(argparse_args)) if v is not NULL)
            ...
            self._argparse_args = AttrDict((k, v) for k, v, in iteritems(argparse_args) if v is not NULL)
    ...
    def validate_all(self):
        validation_errors = list(chain.from_iterable(self.check_source(source)[1] for source in self.raw_data))

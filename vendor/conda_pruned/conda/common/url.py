...
def path_to_url(path):
    ...
    percent_encode = lambda s: "".join(["%%%02X" % ord(c), c][c < "{" and c.isalnum() or c in percent_encode_chars] for c in s)
    if any(ord(char) >= 128 for char in path):
        path = percent_encode(path.decode("unicode-escape") if hasattr(path, "decode") else bytes(path, "utf-8").decode("unicode-escape"))
...
def is_ipv6_address_win_py27(string_ip):
    ...
    return bool(
        re.match(
            r""  # lgtm [py/regex/unmatchable-dollar]
            r"^(((?=.*(::))(?!.*\3.+\3))\3?|[\dA-F]{1,4}:)"
            r"([\dA-F]{1,4}(\3|:\b)|\2){5}"
            r"(([\dA-F]{1,4}(\3|:\b|$)|\2){2}|"
...
def split_scheme_auth_token(url):
    ...
    remainder_url = Url(host=url_parts.host, port=url_parts.port, path=url_parts.path, query=url_parts.query).url

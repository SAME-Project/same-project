...
from ..common.url import Url, has_scheme, is_url, join_url, path_to_url, split_conda_url_easy_parts, split_platform, split_scheme_auth_token, urlparse
    ...
    def __call__(cls, *args, **kwargs):
        if len(args) == 1 and not kwargs:
            ...
            if "channels" in kwargs:
                ...
                name = kwargs["name"]
                channels = tuple(super(ChannelType, cls).__call__(**_kwargs) for _kwargs in kwargs["channels"])
...
class Channel(object):
    ...
    def __init__(self, scheme=None, auth=None, location=None, token=None, name=None, platform=None, package_filename=None):
        self.scheme = scheme
        self.auth = auth
    ...
    def make_simple_channel(channel_alias, channel_url, name=None):
        ...
        if scheme:
            ...
            return Channel(scheme=ca.scheme, auth=ca.auth, location=ca.location, token=ca.token, name=name and name.strip("/") or channel_url.strip("/"))
    ...
    def canonical_name(self):
        ...
        if self.scheme:
            cn = self.__canonical_name = "%s://%s" % (self.scheme, join_url(self.location, self.name))
            return cn
        else:
...
def tokenized_conda_url_startswith(test_url, startswith_url):
    ...
    return tokenized_startswith(norm_url_path(test_url).split("/"), norm_url_path(startswith_url).split("/"))
...
def _get_channel_for_name(channel_name):
    ...
    if channel is not None:
        ...
        return Channel(scheme=ca.scheme, auth=ca.auth, location=ca.location, token=ca.token, name=_stripped, platform=platform)
...
def _read_channel_configuration(scheme, host, port, path):
    ...
    for name, location in sorted(context.migrated_custom_channels.items(), reverse=True, key=lambda x: len(x[0])):
    ...
    for name, channel in sorted(context.custom_channels.items(), reverse=True, key=lambda x: len(x[0])):
        ...
        if tokenized_startswith(test_url.split("/"), that_test_url.split("/")):
            ...
            return (channel.location, join_url(channel.name, subname), scheme, channel.auth, channel.token)
    ...
    return (Url(host=host, port=port, path=bump).url.rstrip("/"), path.strip("/") or None, scheme or None, None, None)


def parse_conda_channel_url(url):
    (scheme, auth, token, platform, package_filename, host, port, path, query) = split_conda_url_easy_parts(context.known_subdirs, url)

    # recombine host, port, path to get a channel_name and channel_location
    (channel_location, channel_name, configured_scheme, configured_auth, configured_token) = _read_channel_configuration(scheme, host, port, path)
    ...
    return Channel(configured_scheme or "https", auth or configured_auth, channel_location, token or configured_token, channel_name, platform, package_filename)
...
def prioritize_channels(channels, with_credentials=True, subdirs=None):
    ...
    channels = concat((Channel(cc) for cc in c._channels) if isinstance(c, MultiChannel) else (c,) for c in (Channel(c) for c in channels))
    result = odict()

...
from .._vendor.auxlib.entity import BooleanField, ComposableField, DictSafeMixin, Entity, EnumField, IntegerField, ListField, NumberField, StringField
...
class TimestampField(NumberField):
    ...
    def box(self, instance, instance_type, val):
        return self._make_seconds(super(TimestampField, self).box(instance, instance_type, val))

    def dump(self, instance, instance_type, val):
        return int(
            self._make_milliseconds(super(TimestampField, self).dump(instance, instance_type, val))
...
class PackageTypeField(EnumField):
    def __init__(self):
        super(PackageTypeField, self).__init__(PackageType, required=False, nullable=True, default=None, default_in_dump=False)
...
class PackageRecord(DictSafeMixin, Entity):
    ...
    legacy_bz2_md5 = StringField(default=None, required=False, nullable=True, default_in_dump=False)
    ...
    def _pkey(self):
        try:
            ...
            __pkey = self.__pkey = [self.channel.canonical_name, self.subdir, self.name, self.version, self.build_number, self.build]
    ...
    license_family = StringField(required=False, nullable=True, default=None, default_in_dump=False)
    ...
    def __str__(self):
        return "%s/%s::%s==%s=%s" % (self.channel.canonical_name, self.subdir, self.name, self.version, self.build)
    ...
    def record_id(self):
        ...
        return "%s/%s::%s-%s-%s" % (self.channel.name, self.subdir, self.name, self.version, self.build)

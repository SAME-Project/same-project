...
from ..common.compat import isiterable, iteritems, itervalues, string_types, text_type, with_metaclass
...
class MatchSpec(object):
    ...
    def merge(cls, match_specs, union=False):
        ...
        merged_specs = []
        mergeable_groups = tuple(concat(itervalues(groupby(lambda s: s.optional, group)) for group in itervalues(name_groups)))
...
class ChannelMatch(GlobStrMatch):
    ...
    def match(self, other):
        ...
        if self._re_match:
            ...
            return self._raw_value.name == _other_val.name or self._raw_value.name == _other_val.canonical_name

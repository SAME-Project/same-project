...
class PrefixGraph(object):
    ...
    def __init__(self, records, specs=()):
        ...
        for node in records:
            ...
            parent_nodes = set(rec for rec in records if any(m.match(rec) for m in parent_match_specs))
    ...
    def remove_youngest_descendant_nodes_with_specs(self):
        ...
        inverted_graph = {node: set(key for key in graph if node in graph[key]) for node in graph}
        youngest_nodes_with_specs = tuple(node for node, children in iteritems(inverted_graph) if not children and node in spec_matches)
    ...
    def prune(self):
        ...
        while True:
            inverted_graph = {node: set(key for key in graph if node in graph[key]) for node in graph}
            prunable_nodes = tuple(node for node, children in iteritems(inverted_graph) if not children and node not in spec_matches)
    ...
    def _toposort_raise_on_cycles(cls, graph):
        ...
        while True:
            no_parent_nodes = IndexedSet(sorted((node for node, parents in iteritems(graph) if len(parents) == 0), key=lambda x: x.name))
    ...
    def _toposort_pop_key(graph):
        ...
        """
        node_with_fewest_parents = sorted((len(parents), node.dist_str(), node) for node, parents in iteritems(graph))[0][2]

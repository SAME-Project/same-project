...
class History(object):
    ...
    def update(self):
        ...
        try:
            try:
                ...
                warnings.warn("Error in %s: %s" % (self.path, e), CondaHistoryWarning)
                return
    ...
    def object_log(self):
        ...
        for i, (date, content, unused_com) in enumerate(self.parse()):
            ...
            # history event
            event = {"date": date, "rev": i, "install": [], "remove": [], "upgrade": [], "downgrade": []}
            added = {}
            removed = {}
            if is_diff(content):
                for pkg in content:
                ...
                for name in sorted(changed):
                    old = removed[name]
                    new = added[name]
                    details = {"old": "-".join((name,) + old), "new": "-".join((name,) + new)}

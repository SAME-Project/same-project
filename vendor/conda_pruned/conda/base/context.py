...
from ..common.configuration import Configuration, ConfigurationLoadError, MapParameter, ParameterLoader, PrimitiveParameter, SequenceParameter, ValidationError
...
def ssl_verify_validation(value):
    if isinstance(value, string_types):
        if not isfile(value) and not isdir(value):
            return "ssl_verify value '%s' must be a boolean, a path to a " "certificate bundle file, or a path to a directory containing " "certificates of trusted CAs." % value
    return True
...
class Context(Configuration):
    ...
    default_python = ParameterLoader(PrimitiveParameter(default_python_default(), element_type=string_types + (NoneType,), validation=default_python_validation))
    ...
    _default_threads = ParameterLoader(PrimitiveParameter(0, element_type=int), aliases=("default_threads",))
    _repodata_threads = ParameterLoader(PrimitiveParameter(0, element_type=int), aliases=("repodata_threads",))
    _verify_threads = ParameterLoader(PrimitiveParameter(0, element_type=int), aliases=("verify_threads",))
    # this one actually defaults to 1 - that is handled in the property below
    _execute_threads = ParameterLoader(PrimitiveParameter(0, element_type=int), aliases=("execute_threads",))

    # Safety & Security
    _aggressive_update_packages = ParameterLoader(
        SequenceParameter(PrimitiveParameter("", element_type=string_types), DEFAULT_AGGRESSIVE_UPDATE_PACKAGES), aliases=("aggressive_update_packages",)
    )
    ...
    _signing_metadata_url_base = ParameterLoader(PrimitiveParameter(None, element_type=string_types + (NoneType,)), aliases=("signing_metadata_url_base",))
    ...
    disallowed_packages = ParameterLoader(SequenceParameter(PrimitiveParameter("", element_type=string_types), string_delimiter="&"), aliases=("disallow",))
    ...
    _envs_dirs = ParameterLoader(
        SequenceParameter(PrimitiveParameter("", element_type=string_types), string_delimiter=os.pathsep), aliases=("envs_dirs", "envs_path"), expandvars=True
    )
    _pkgs_dirs = ParameterLoader(SequenceParameter(PrimitiveParameter("", string_types)), aliases=("pkgs_dirs",), expandvars=True)
    ...
    ssl_verify = ParameterLoader(PrimitiveParameter(True, element_type=string_types + (bool,), validation=ssl_verify_validation), aliases=("verify_ssl",), expandvars=True)
    client_ssl_cert = ParameterLoader(PrimitiveParameter(None, element_type=string_types + (NoneType,)), aliases=("client_cert",), expandvars=True)
    client_ssl_cert_key = ParameterLoader(PrimitiveParameter(None, element_type=string_types + (NoneType,)), aliases=("client_cert_key",), expandvars=True)
    proxy_servers = ParameterLoader(MapParameter(PrimitiveParameter(None, string_types + (NoneType,))), expandvars=True)
    ...
    _channel_alias = ParameterLoader(PrimitiveParameter(DEFAULT_CHANNEL_ALIAS, validation=channel_alias_validation), aliases=("channel_alias",))
    channel_priority = ParameterLoader(PrimitiveParameter(ChannelPriority.FLEXIBLE))
    _channels = ParameterLoader(
        SequenceParameter(PrimitiveParameter("", element_type=string_types), default=(DEFAULTS_CHANNEL_NAME,)),
        aliases=(
            "channels",
            "channel",
        ),
    ...
    _custom_channels = ParameterLoader(MapParameter(PrimitiveParameter("", element_type=string_types), DEFAULT_CUSTOM_CHANNELS), aliases=("custom_channels",), expandvars=True)
    _custom_multichannels = ParameterLoader(MapParameter(SequenceParameter(PrimitiveParameter("", element_type=string_types))), aliases=("custom_multichannels",), expandvars=True)
    _default_channels = ParameterLoader(SequenceParameter(PrimitiveParameter("", element_type=string_types), DEFAULT_CHANNELS), aliases=("default_channels",), expandvars=True)
    _migrated_channel_aliases = ParameterLoader(SequenceParameter(PrimitiveParameter("", element_type=string_types)), aliases=("migrated_channel_aliases",))
    migrated_custom_channels = ParameterLoader(MapParameter(PrimitiveParameter("", element_type=string_types)), expandvars=True)  # TODO: also take a list of strings
    ...
    whitelist_channels = ParameterLoader(SequenceParameter(PrimitiveParameter("", element_type=string_types)), expandvars=True)
    restore_free_channel = ParameterLoader(PrimitiveParameter(False))
    repodata_fns = ParameterLoader(SequenceParameter(PrimitiveParameter("", element_type=string_types), ("current_repodata.json", REPODATA_FN)))
    _use_only_tar_bz2 = ParameterLoader(PrimitiveParameter(None, element_type=(bool, NoneType)), aliases=("use_only_tar_bz2",))
    ...
    always_yes = ParameterLoader(PrimitiveParameter(None, element_type=(bool, NoneType)), aliases=("yes",))
    ...
    _verbosity = ParameterLoader(PrimitiveParameter(0, element_type=int), aliases=("verbose", "verbosity"))
    ...
    anaconda_upload = ParameterLoader(PrimitiveParameter(None, element_type=(bool, NoneType)), aliases=("binstar_upload",))
    _croot = ParameterLoader(PrimitiveParameter(""), aliases=("croot",))
    _conda_build = ParameterLoader(MapParameter(PrimitiveParameter("", element_type=string_types)), aliases=("conda-build", "conda_build"))
    ...
    def __init__(self, search_path=None, argparse_args=None):
        ...
        super(Context, self).__init__(search_path=search_path, app_name=APP_NAME, argparse_args=argparse_args)

    def post_build_validation(self):
        errors = []
        if self.client_ssl_cert_key and not self.client_ssl_cert:
            error = ValidationError("client_ssl_cert", self.client_ssl_cert, "<<merged>>", "'client_ssl_cert' is required when 'client_ssl_cert_key' " "is defined")
            errors.append(error)
        if self.always_copy and self.always_softlink:
            error = ValidationError("always_copy", self.always_copy, "<<merged>>", "'always_copy' and 'always_softlink' are mutually exclusive. " "Only one can be set to 'True'.")
            errors.append(error)
        return errors
        ...
        if context.dev:
            return OrderedDict(
                [
                    ...
                    ("PYTHONPATH", os.path.dirname(CONDA_PACKAGE_ROOT) + "{}{}".format(os.pathsep, os.environ.get("PYTHONPATH", ""))),
                    ("_CE_M", "-m"),
                    ("_CE_CONDA", "conda"),
            ...
            # error-on-undefined.
            return OrderedDict([("CONDA_EXE", os.path.join(sys.prefix, bin_dir, exe)), ("_CE_M", ""), ("_CE_CONDA", ""), ("CONDA_PYTHON_EXE", sys.executable)])
    ...
    def migrated_channel_aliases(self):
        ...
        return tuple(
            ...
            for location, scheme, auth, token in (split_scheme_auth_token(c) for c in self._migrated_channel_aliases)
        )

    @property
    def prefix_specified(self):
        return self._argparse_args.get("prefix") is not None or self._argparse_args.get("name") is not None
    ...
    def custom_multichannels(self):
        ...
        reserved_multichannels = odict((name, tuple(Channel.make_simple_channel(self.channel_alias, url) for url in urls)) for name, urls in iteritems(reserved_multichannel_urls))
        custom_multichannels = odict((name, tuple(Channel.make_simple_channel(self.channel_alias, url) for url in urls)) for name, urls in iteritems(self._custom_multichannels))
    ...
    def custom_channels(self):
        ...
        custom_channels = (Channel.make_simple_channel(self.channel_alias, url, name) for name, url in iteritems(self._custom_channels))
    ...
    def channels(self):
        ...
        if self._argparse_args and "override_channels" in self._argparse_args and self._argparse_args["override_channels"]:
            ...
            elif not (self._argparse_args and "channel" in self._argparse_args and self._argparse_args["channel"]):
        ...
        if self._argparse_args and "channel" in self._argparse_args:
            ...
            channel_in_config_files = any("channels" in context.raw_data[rc_file].keys() for rc_file in self.config_files)
    ...
    #     return (hasattr(conda_package_handling.api, "libarchive_enabled") and not conda_package_handling.api.libarchive_enabled) or self._use_only_tar_bz2 or use_only_tar_bz2

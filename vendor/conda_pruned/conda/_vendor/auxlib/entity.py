...
from .compat import integer_types, isiterable, iteritems, itervalues, odict, string_types, text_type, with_metaclass
...
class Field(object):
    ...
    def __init__(self, default=NULL, required=True, validation=None, in_dump=True, default_in_dump=True, nullable=False, immutable=False, aliases=()):
    ...
    def name(self):
        try:
            ...
            log.error("The name attribute has not been set for this field. " "Call set_name at class creation time.")
            raise
    ...
    def __delete__(self, instance):
        if self.immutable and instance._initd:
            ...
            raise AttributeError("The {0} field is required and cannot be deleted.".format(self.name))
...
class EnumField(Field):
    def __init__(self, enum_class, default=NULL, required=True, validation=None, in_dump=True, default_in_dump=True, nullable=False, immutable=False, aliases=()):
        if not issubclass(enum_class, Enum):
            raise ValidationError(None, msg="enum_class must be an instance of Enum")
        self._type = enum_class
        super(EnumField, self).__init__(default, required, validation, in_dump, default_in_dump, nullable, immutable, aliases)
...
class ListField(Field):
    ...
    def __init__(self, element_type, default=NULL, required=True, validation=None, in_dump=True, default_in_dump=True, nullable=False, immutable=False, aliases=()):
        self._element_type = element_type
        super(ListField, self).__init__(default, required, validation, in_dump, default_in_dump, nullable, immutable, aliases)

    def box(self, instance, instance_type, val):
        if val is None:
            return None
        elif isinstance(val, string_types):
            raise ValidationError("Attempted to assign a string to ListField {0}" "".format(self.name))
        elif isiterable(val):
            ...
            raise ValidationError(val, msg="Cannot assign a non-iterable value to " "{0}".format(self.name))
    ...
    def validate(self, instance, val):
        ...
        if val:
            et = self._element_type
            self._type(Raise(ValidationError(self.name, el, et)) for el in val if not isinstance(el, et))
        return val
...
class MapField(Field):
    ...
    def __init__(self, default=NULL, required=True, validation=None, in_dump=True, default_in_dump=True, nullable=False, immutable=True, aliases=()):
        super(MapField, self).__init__(default, required, validation, in_dump, default_in_dump, nullable, immutable, aliases)
...
class ComposableField(Field):
    def __init__(self, field_class, default=NULL, required=True, validation=None, in_dump=True, default_in_dump=True, nullable=False, immutable=False, aliases=()):
        self._type = field_class
        super(ComposableField, self).__init__(default, required, validation, in_dump, default_in_dump, nullable, immutable, aliases)
...
class EntityType(type):
    ...
    def __new__(mcs, name, bases, dct):
        ...
        non_field_keys = (key for key, value in iteritems(dct) if not isinstance(value, Field) and not key.startswith("__"))
        entity_subclasses = EntityType.__get_entity_subclasses(bases)
        if entity_subclasses:
            keys_to_override = [key for key in non_field_keys if any(isinstance(base.__dict__.get(key), Field) for base in entity_subclasses)]
    ...
    def __init__(cls, name, bases, attr):
        ...
        for clz in reversed(type.mro(cls)):
            clz_fields = ((name, field.set_name(name)) for name, field in iteritems(clz.__dict__) if isinstance(field, Field))
...
class Entity(object):
        ...
        for key, field in iteritems(self.__fields__):
            try:
                ...
                if alias is not None:
                    ...
                    raise ValidationError(key, msg="{0} requires a {1} field. Instantiated with " "{2}".format(self.__class__.__name__, key, kwargs))
    ...
    def from_objects(cls, *objects, **override_fields):
        init_vars = dict()
        search_maps = tuple(AttrDict(o) if isinstance(o, dict) else o for o in ((override_fields,) + objects))
    ...
    def validate(self):
        ...
        try:
            reduce(lambda _, name: getattr(self, name), (name for name, field in iteritems(self.__fields__) if field.required))
    ...
    def __repr__(self):
        ...
        kwarg_str = ", ".join("{0}={1}".format(key, _val(key)) for key in sorted(self.__dict__, key=_sort_helper) if _valid(key))
    ...
    def dump(self):
        return odict(
            ...
            for field, value in ((field, getattr(self, field.name, NULL)) for field in self.__dump_fields())
    ...
    def __dump_fields(cls):
        if "__dump_fields_cache" not in cls.__dict__:
            cls.__dump_fields_cache = tuple(field for field in itervalues(cls.__fields__) if field.in_dump)
    ...
    def __eq__(self, other):
        ...
        return all(getattr(self, field, rando_default) == getattr(other, field, rando_default) for field in self.__fields__)
...
class ImmutableEntity(Entity):
    def __setattr__(self, attribute, value):
        if self._initd:
            raise AttributeError("Assignment not allowed. {0} is immutable.".format(self.__class__.__name__))
        super(ImmutableEntity, self).__setattr__(attribute, value)

    def __delattr__(self, item):
        if self._initd:
            raise AttributeError("Deletion not allowed. {0} is immutable.".format(self.__class__.__name__))

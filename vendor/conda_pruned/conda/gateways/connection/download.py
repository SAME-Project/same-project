...
from . import ConnectionError, HTTPError, InsecureRequestWarning, InvalidSchema, SSLError, RequestsProxyError
...
from ...exceptions import BasicClobberError, CondaDependencyError, CondaHTTPError, ChecksumMismatchError, maybe_raise, ProxyError
...
# def download(url, target_full_path, md5=None, sha256=None, size=None, progress_update_callback=None):
...
#                 """
#                 )
#                 raise CondaError(message, url=url, target_path=target_full_path, content_length=content_length, downloaded_bytes=streamed_bytes)
...
#                 log.debug("%s mismatch for download: %s (%s != %s)", checksum_type, url, actual_checksum, checksum)
#                 raise ChecksumMismatchError(url, target_full_path, checksum_type, checksum, actual_checksum)
#         if size is not None:
...
#             help_message, url, getattr(e.response, "status_code", None), getattr(e.response, "reason", None), getattr(e.response, "elapsed", None), e.response, caused_by=e
#         )
...
def download_text(url):
    ...
    try:
        ...
        raise CondaHTTPError(help_message, url, status_code, getattr(e.response, "reason", None), getattr(e.response, "elapsed", None), e.response, caused_by=e)

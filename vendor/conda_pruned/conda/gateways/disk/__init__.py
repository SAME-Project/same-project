...
def exp_backoff_fn(fn, *args, **kwargs):
    ...
    def sleep_some(n, exc):
        ...
        log.trace("retrying %s/%s %s() in %g sec", basename(caller_frame.f_code.co_filename), caller_frame.f_lineno, fn.__name__, sleep_time)
        sleep(sleep_time)
...
def mkdir_p_sudo_safe(path):
    ...
    if not on_win:
        ...
        try:
            ...
            log.trace("Failed to set permissions to 2775 on %s (%d %d)", path, e.errno, errorcode[e.errno])
            pass

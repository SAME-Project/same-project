...
if not (on_win and PY2):
    ...
    def readlink(link):
        ...
        """
        handle = CreateFile(link, 0, 0, None, OPEN_EXISTING, FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, None)
...
class CrossPlatformStLink(object):
    ...
    def _windows_st_nlink(cls, path):  # pragma: unix no cover
        ...
        if st_nlink != 0:
            ...
            OPEN_EXISTING = 3
            hfile = cls.CreateFile(path, GENERIC_READ, FILE_SHARE_READ, None, OPEN_EXISTING, 0, None)
            if hfile is None:
    ...
    def _initialize(cls):
        if not on_win:
            ...
            cls.CreateFile.argtypes = [c_wchar_p, DWORD, DWORD, c_void_p, DWORD, DWORD, HANDLE]

...
spec_pat = re.compile(
    ...
    r")?$",
    re.VERBOSE,
)  # lgtm [py/regex/unmatchable-dollar]
...
def spec_from_line(line):
    ...
    if cc:
        ...
        if pc.startswith("~= "):
            assert pc.count("~=") == 1, "Overly complex 'Compatible release' spec not handled {}".format(line)
        ...
        try:
            for line in open(path):
                ...
                if spec is None:
                    ...
                    raise CondaValueError("could not parse '%s' in: %s" % (line, url))
                specs.append(spec)
        except IOError as e:
